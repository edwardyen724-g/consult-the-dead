...
| # | Check | Pass condition |
|---|-------|---------------|
| 17 | Inline CTA strip | Visible on desktop (> 720 px): amber left-border panel, "Begin your own agon", "Start a free agon →" button |
| 18 | Sticky CTA bar | Visible on mobile (≤ 720 px): fixed bottom bar, same copy and button |
| 19 | CTA href | Both links point to `/agora?utm_source=share&utm_campaign=agon_share&utm_content=<share_id>` |
| 20 | "Run your own agon →" footer | Button at bottom of page links to `/agora` (with UTM if present in incoming URL) |
| 21 | UTM passthrough | If page was opened with `?utm_campaign=X&utm_content=Y`, footer CTA preserves those params |
| 22 | Generated-by footer | Italic serif "Generated by Consult The Dead — The Agora" with dotted link |

#### Print / OG
| # | Check | Pass condition |
|---|-------|---------------|
| 23 | Print — CTA hidden | Both ShareCtaStrip instances absent in print preview / saved PDF |
| 24 | Print — attribution | "Generated by …" footer line visible in print |
| 25 | OG title | `<title>` is `Agon: <truncated topic> · Consult The Dead` |
| 26 | OG description | Twitter / OG description is the topic trimmed to ≤ 180 chars |
| 27 | Canonical URL | `<link rel="canonical">` equals `https://www.consultthedead.com/agora/a/<share_id>` |

#### Error state
| # | Check | Pass condition |
|---|-------|---------------|
| 28 | Missing share_id | `/agora/a/nonexistent` returns a 404 page (not a 500) |
