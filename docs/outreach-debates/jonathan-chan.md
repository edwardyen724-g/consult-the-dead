...
- Step 1 — This week: Write a one-page document stating that revenue-per-hour-of-personal-attention is the sole selection criterion, sign and date it, then calculate the actual number for each business using the last 90 days of data. Do not look at the output until the document is signed.
- Step 2 — Days 8–14: Identify the condemned business and draft a concrete shutdown or sale plan — customer transitions, legal wind-down, asset disposition — with a hard completion date no later than 30 days from today.
- Step 3 — Days 15–30: Execute the shutdown completely and without a revenue taper or soft exit. Redirect all freed attention to the surviving business and set a 90-day growth target that would have been impossible under divided command.
