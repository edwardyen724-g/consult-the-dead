...
- Step 1 (this week) — Rank all apps by gross revenue, net margin, and growth rate using the last 90 days of actual data. Identify the single winner unambiguously. Write down the decision in one sentence.
- Step 2 (weeks 2–4) — Sunset or place on minimal maintenance all non-winner apps. Redirect all your time and any shared infrastructure costs to the one app. Generate a clean P&L for it in isolation.
- Step 3 (days 30–120) — Run the single app for 90 days, tracking weekly revenue, churn, CAC, and LTV with no portfolio noise. At day 90, make the funding decision based on the trend line: if growth is compounding cleanly, open investor conversations from surplus; if not, stay bootstrapped and iterate.
