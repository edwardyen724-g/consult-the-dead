...
- Step 1 — This week: Build a single dashboard that tracks MRR, churn rate, gross margin, and owner time-per-week for each of the three companies separately, so you can see which company is subsidizing or obscuring the others.
- Step 2 — Within 30 days: Document every recurring decision you make across all three companies, then identify the top three tasks per company that only you currently perform — these become your first delegation or automation targets.
- Step 3 — By month six: Validate that each company has run for at least 30 consecutive days without a decision requiring your direct input, review the quantitative anomalies your measurement surfaced, and only then write a one-page acquisition profile specifying exactly what type of company belongs in this portfolio and why.
