...
- Step 1 (Day 1-2) — Write a single-page causal map of the first product's failure, naming exactly which load-bearing mechanism broke (distribution channel, pricing model, co-founder dependency, customer pain severity) and marking explicitly whether each broken mechanism is present or absent in your current $30K MRR build.
- Step 2 (Day 3) — For every feature candidate on your roadmap, write one honest sentence answering: would I avoid this feature if the first product had succeeded? Any 'yes' answer is flagged and cannot advance without external customer validation — this is Marcus's prophylactic step, performed after the causal map so trauma cannot redirect the forensic categories.
- Step 3 (Day 4-7) — Deploy a structured customer survey to your current paying users measuring specific capability gaps, weighted by retention cohort and expansion revenue; rank the output numerically and let the top three items become your next sprint — no further introspection required once the ranked data exists.
