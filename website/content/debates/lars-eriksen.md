...
- Step 1 (this week) — Block two days to audit every active Fund III company: log actual hours per company per month for the past quarter, identify which positions are in high-demand phases, and produce a single number representing available cognitive hours per week if Fund IV were live simultaneously.
- Step 2 (within two weeks) — Use the audit output to set a non-negotiable fund size ceiling and a hard portfolio company cap, then draft the contractual language for both into a term sheet template before you speak to a single LP.
- Step 3 (weeks three through six) — Open the LP roadshow only after steps one and two are complete, lead with the audit-derived constraint as a feature not a limitation, and price your target check size to lead every round you enter — the cap is your competitive positioning narrative, not a concession.
