...
function consumeLocalThrottle(key: string, limit: number) {
  const now = Date.now();
  const bucket = localThrottle.get(key);
  if (!bucket || bucket.resetAt <= now) {
    const freshBucket = { count: 1, resetAt: now + CONTACT_WINDOW_MS };
...
export async function POST(request: Request) {
  ...
  if (throttleError) {
    ...
    const ipThrottle = consumeLocalThrottle(ipKey, CONTACT_IP_LIMIT);
    const emailThrottle = consumeLocalThrottle(emailKey, CONTACT_EMAIL_LIMIT);
