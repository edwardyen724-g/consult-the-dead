...
- CI Node.js runtime upgraded to Node.js 24 across all jobs (PR #363): lint, test, and build CI jobs now run on Node.js 24 to stay ahead of the Node.js 20 EOL on June 2, 2026.
- "What Would X Say" articles batch 1 published (PR #364): three new insight pages live under `/insights` — `what-would-sun-tzu-say-about-tariffs-and-trade-wars`, `what-would-machiavelli-say-about-firing-someone-you-respect`, and `what-would-marcus-aurelius-say-about-burnout`; annotation blueprints added for each entry.
- ProofStrip null-render when no live data is available (PR #365): `ProofStrip` component now returns `null` when no live `data` prop is provided instead of displaying fabricated subscriber and session counts; `PROOF_STRIP_FALLBACK` constant removed from `website/src/lib/proof-strip.ts`; ProofStrip removed from Footer where no server-side live data is available; tests updated to assert null rendering when data is absent.
